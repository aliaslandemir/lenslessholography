# -*- coding: utf-8 -*-
"""
Created on Thu Jan 11 23:00:05 2024
...
def FT2Dc(in_val):
    ...
    for ii in range(Nx):
        for jj in range(Ny):
            f1[ii, jj] = np.exp(1j * np.pi * (ii + jj))
...
def Propagator(N, lambda_val, area, z):
    ...
    for ii in range(N):
        for jj in range(N):
            alpha = lambda_val * (ii - N / 2 - 1) / area
            beta = lambda_val * (jj - N / 2 - 1) / area
            if alpha**2 + beta**2 <= 1:
